...
/**
 * Servlet implementation class ShowResearchListServlet
 */
/** 
 * @ClassName: ShowResearchListServlet 
 * @Description: TODO 获取科学研究列表 
 * @date 2016年12月29日 下午4:59:48  
 */
@WebServlet("/ShowResearchListServlet")
public class ShowResearchListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ShowResearchListServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//根据信息类型获取信息的总记录数值
		int rCount=new CountInfoDao().getResearchCount(type);
		//设置每页显示的数据量
		int item_pre_page=CommonConfig.HOME_PAGESIZE;
		//计算偏移量
		int position=(p-1)*item_pre_page;
		//根据偏移量获取相应的信息
		ArrayList<ResearchBean> research=(ArrayList<ResearchBean>) new AdminSimpleInfoDaoImpl<ResearchBean>().getSimpleResearchInfo(type,position,item_pre_page);
		...
		ph.setIndex(p);
		//设置url
        String url="./researchlist.ado?type='"+type+"'&p=";
        //去掉 url中的单引号
        url=url.replace("'", "");
        ph.setPath(url); 
		//获取导航条
		...
		request.getRequestDispatcher("./admin/research/researchList.jsp").forward(request, response); 
	}
