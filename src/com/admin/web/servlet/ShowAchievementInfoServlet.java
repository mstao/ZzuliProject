...
/**
 * Servlet implementation class ShowAchievementInfoServlet
 */
/** 
 * @ClassName: ShowAchievementInfoServlet 
 * @Description: TODO  显示 科研成果信息
 * @date 2016年12月30日 下午8:06:10  
 */
@WebServlet("/ShowAchievementInfoServlet")
public class ShowAchievementInfoServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ShowAchievementInfoServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//根据信息类型获取信息的总记录数值
		int aCount=new CountInfoDao().getAchievement(type);
		...
		//根据偏移量获取对应数据
		ArrayList<AchievementBean> achievement=(ArrayList<AchievementBean>) new AdminSimpleInfoDaoImpl<AchievementBean>().getSimpleAchievementInfo(type, position, item_pre_page);
		//分页工具类
		...
		phelper.setIndex(p);
		//设置url
        String url="./showachievement.ado?type='"+type+"'&p=";
        //去掉 url中的单引号
        url=url.replace("'", "");
		phelper.setPath(url); 
		//获取导航条
	    ...
	    request.getRequestDispatcher("./admin/achievement/showAchievement.jsp").forward(request, response);
